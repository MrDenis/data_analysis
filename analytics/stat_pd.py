...
def output_formatted (frame):
   ...
   for key, group in grouped:
      ...
      for app_no, app_no_group in group.groupby('CAT3'):
         print ("     " + app_no + "\n     =====\n")
         for row_num, row in app_no_group.iterrows():
...
def process_csv(filename):
   ...
   df = df.apply(lambda x : pd.to_numeric(x) if x.name in ['SQUARE_TOTAL', 'PRICE_PER_1M2','PRICE_PER_1M2', 'PRICE'] else x)
   fvm = importlib.import_module("filters.90k_newb")

   df = df.loc[df['CAT3'].isin(fvm.filter_values['cat3']) &
               df['NEW_BUILD'].isin(fvm.filter_values['new_build']) &
               df['CAT4'].isin(fvm.filter_values['cat4'])
   ]

   df = df.loc[(df['SQUARE_TOTAL'] != None) & (df['SQUARE_TOTAL'] < fvm.filter_values['sq_total_max'])
               & (
                  (df['PRICE_PER_1M2'] != None)
                  & (
                     ( (df['CURRENCY'] == 'USD') & (df['PRICE_PER_1M2'] < fvm.filter_values['price_per_1m2_usd']) ) | 
                     ( (df['CURRENCY'] == 'UAH') & (df['PRICE_PER_1M2'] < fvm.filter_values['price_per_1m2_uah']) )
                  )
                  & (
                  ( (df['CURRENCY']=='USD') & (df['PRICE'] < fvm.filter_values['price_usd'])) | 
                  ( (df['CURRENCY']=='UAH') & (df['PRICE'] < fvm.filter_values['price_uah']))
                  )
               )
               ]
